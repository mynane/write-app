...
// update config
#[tauri::command]
pub fn patch_config(item: ConfigsInner, configs_state: State<'_, ConfigsState>) -> ConfigsInner {
    let mut configs = configs_state.0.lock().unwrap();
    configs.patch_item(item).unwrap();
    configs.0.clone()
}
